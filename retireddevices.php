...
$value = 1;
$query="select * from devices where retire='".$value."'";
...
<body>
	...
	<h1 style="text-align:center">Retired Devices Data</h1>
	
	<h3 style="text-align:center; border-weight: 0px;">Record</h3>
	
	<h4 style="text-align:center; border-weight: 0px;"><?php
echo "Today date is ".$date."<br>";?></h4>
		...
		?>
				...
				<tr>
					...
					<td>						<?php
						...
						echo $arraydate;
						// $datecomp = $row['Date'];
// 						foreach($datecomp as $a){
// 							if($a(mktime(0,0,0, date("m"),date("d"),date("Y")+3)) < $date){
// 								echo '<td style="background-color:yellow">';
// 								echo $Date;
// 							}
// 							else if($a(mktime(0,0,0, date("m"),date("d"),date("Y")+3) < $date)){
